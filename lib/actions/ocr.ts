...
export async function extractLabResultTable(
  ...
  if (looksLikePdf) {
    return { error: 'PDF belgeler için otomatik metne çevirme henüz desteklenmiyor — bu özellik sadece fotoğraf/resim belgeler içindir.' };
  }
  ...
  try {
    ...
    if (!rawText.trim()) {
      return { error: 'Belgede okunabilir bir yazı bulunamadı. Fotoğrafın net, düz ve iyi aydınlatılmış olduğundan emin olup tekrar deneyin.' };
    }
...
export async function saveDigitalLabResult(
  ...
  try {
    ...
    pdfBytes = await buildLabResultPdf({
    ...
    return { error: 'PDF oluşturulamadı: ' + (err?.message ?? String(err)) };
  }
