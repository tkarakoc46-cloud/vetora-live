...
export function LabResultUploadForm({ patientId }: { patientId: string }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!file) {
      setError('Yüklenecek dosya seçilmedi. Önce "Dosya Seç" ile bir belge seçmelisiniz.');
      return;
    }
    ...
    try {
      ...
      if (uploadError) {
        ...
        console.error('lab-result uploadToSignedUrl error', uploadError);
      ...
      // eslint-disable-next-line no-console
      console.error('lab-result upload unexpected error', err);
    ...
    <form onSubmit={handleSubmit} className="field space-y-2 mb-3">
      ...
      {success && !error && (
        <div className="text-xs text-green font-semibold bg-green-50 border border-green/30 rounded-lg px-3 py-2">
          ✓ Belge başarıyla yüklendi ve arşive kaydedildi.
        </div>
      )}
      <button
        type="submit"
